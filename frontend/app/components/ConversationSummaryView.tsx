...
import { ConversationSummary, KeyPoint, SummaryService, SummaryRequest } from '../services/summaryService';
...
export const ConversationSummaryView: React.FC<ConversationSummaryProps> = ({
    ...
    const loadSummaries = async () => {
        try {
            const conversationSummaries = await summaryService.getConversationSummaries(conversationId);
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="bg-white shadow rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="text-sm">
                            Completion: {(selectedSummary.completion_percentage * 100).toFixed(0)}%
                        </div>
                    </div>
                    ...
                    {selectedSummary.key_points && renderKeyPoints(selectedSummary.key_points)}
                        ...
                        <div className="mt-4 flex items-center">
                            ...
                            <div className="ml-2 flex items-center">
                                {selectedSummary.sentiment_score > 0 ? (
                                    <span className="text-green-600">Positive ({(selectedSummary.sentiment_score * 100).toFixed(0)}%)</span>
                                ) : selectedSummary.sentiment_score < 0 ? (
                                    <span className="text-red-600">Negative ({(Math.abs(selectedSummary.sentiment_score) * 100).toFixed(0)}%)</span>
                                ) : (
                    ...
                    {(selectedSummary.categories.length > 0 || selectedSummary.tags.length > 0) && (
            ...
            {summaries.length > 1 && (
                <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-700 mb-2">Previous Summaries</h4>
                    <div className="space-y-2">
